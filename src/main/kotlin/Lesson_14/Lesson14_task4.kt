...
class Satellite(
    name: String, hasAtmosphere: Boolean, suitableForLanding: Boolean
) : CelestialBody(name, hasAtmosphere, suitableForLanding)

fun main() {
    val planet = Planet("Земля", hasAtmosphere = true, suitableForLanding = true)
    val moon = Satellite("Луна", hasAtmosphere = false, suitableForLanding = false)
    val phobos = Satellite("Фобос", hasAtmosphere = false, suitableForLanding = false)
